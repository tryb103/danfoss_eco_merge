...
namespace esphome
{
    namespace danfoss_eco
    {
        ...
        {
            // BUG FIX (both forks, consensus): the original passed a hard-coded `8` to DeviceData,
            // so `length` was always 8 even for 16-byte settings. That truncated the settings write
            // (only first 8 of 16 bytes encrypted -> "last 8 bytes garbage") AND overflowed the
            // 8-byte stack buffer in WritableProperty::write_request (pack() writes up to byte 13).
            // Passing `l` through makes `length` correct (8 for temperature, 16 for settings).
